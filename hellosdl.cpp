...
void renderPaddle(const Paddle& p) {
    SDL_FRect rect = { (float)p.x, (float)p.y, (float)PADDLE_WIDTH, (float)PADDLE_HEIGHT };
...
void renderBall(const Ball& b) {
    SDL_FRect rect = { (float)b.x, (float)b.y, (float)BALL_SIZE, (float)BALL_SIZE };
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_EVENT_KEY_DOWN) {
                switch (e.key.scancode) {
                    case SDL_SCANCODE_W: left.vy = -PADDLE_SPEED; break;
                    case SDL_SCANCODE_S: left.vy = PADDLE_SPEED; break;
                    case SDL_SCANCODE_UP: right.vy = -PADDLE_SPEED; break;
                    case SDL_SCANCODE_DOWN: right.vy = PADDLE_SPEED; break;
                }
            }
            if (e.type == SDL_EVENT_KEY_UP) {
                switch (e.key.scancode) {
                    case SDL_SCANCODE_W: case SDL_SCANCODE_S: left.vy = 0; break;
                    case SDL_SCANCODE_UP: case SDL_SCANCODE_DOWN: right.vy = 0; break;
                }
            }
        }
